...
// Calcular total del carrito aplicando descuentos
$stmt = $conn->prepare("SELECT products.price, products.descuento, cart.quantity 
                        FROM cart LEFT JOIN products ON products.id = cart.product_id 
                        WHERE cart.user_id = :user_id");
$stmt->execute(['user_id' => $user['id']]);
$total = 0;
foreach ($stmt as $trow) {
	$precio_final = precioConDescuento($trow['price'], $trow['descuento']);
	$total += $precio_final * $trow['quantity'];
}
