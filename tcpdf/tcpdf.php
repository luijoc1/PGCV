		...
		if ($this->encryptdata['V'] >= 4) {
			...
			if (isset($this->encryptdata['CF']) AND !empty($this->encryptdata['CF'])) {
				...
				if (isset($this->encryptdata['CF']['Length']) AND !empty($this->encryptdata['CF']['Length'])) {
					//La longitud de bits de la clave de cifrado.
					$out .= ' /Length '.$this->encryptdata['CF']['Length'];
				}
				$out .= ' >> >>';
			}
			//El nombre del filtro de cripta que se utilizará de forma predeterminada al descifrar secuencias.
			$out .= ' /StmF /'.$this->encryptdata['StmF'];
			// El nombre del filtro de cripta que se utilizará al descifrar todas las cadenas del documento.
			$out .= ' /StrF /'.$this->encryptdata['StrF'];
			if (isset($this->encryptdata['EFF']) AND !empty($this->encryptdata['EFF'])) {
				//El nombre del filtro de cifrado que se utilizará al cifrar flujos de archivos incrustados que no tienen su propio especificador de filtro de cifrado.
				$out .= ' /EFF /'.$this->encryptdata[''];
			}
		}
		// Entradas de diccionario de cifrado adicionales para el controlador de seguridad estándar
		if ($this->encryptdata['pubkey']) {
