...
class TCPDF_FONTS {
	...
	public static function addTTFfont($fontfile, $fonttype='', $enc='', $flags=32, $outpath='', $platid=3, $encid=1, $addcbbox=false, $link=false) {
		...
		if ($fmetric['type'] == 'Type1') {
			...
			// obtener el número de entradas hMetric en la tabla hmtx
			$numberOfHMetrics = TCPDF_STATIC::_getUSHORT($font, $offset);
			// ---------- obtener datos maxp ----------
			$offset = $table['maxp']['offset'];
			$offset += 4; // omitir el número de versión de la tabla
			// obtén el número de glifos en la fuente.
			$numGlyphs = TCPDF_STATIC::_getUSHORT($font, $offset);
			// ---------- get CIDToGIDMap ----------
			$ctg = array();
			foreach ($encodingTables as $enctable) {
				// obtener solo el ID de plataforma y el ID de codificación especificados
