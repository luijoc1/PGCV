...
if (isset($_SESSION['user'])) {
	...
	try {
		...
		foreach ($stmt as $row) {
			...
			$precio_html = ($row['descuento'] > 0)
				? "<small style='text-decoration:line-through; color:#999;'>&#36; " . number_format($row['price'], 2) . "</small><br>
	...
	if (count($_SESSION['cart']) != 0) {
		...
		foreach ($_SESSION['cart'] as $key => $row) {
			...
			$precio_html = ($product['descuento'] > 0)
				? "<small style='text-decoration:line-through; color:#999;'>&#36; " . number_format($product['price'], 2) . "</small><br>
       <b style='color:#e74c3c;'>&#36; " . number_format($precio_final, 2) . "</b>
       <span style='background:#e74c3c; color:#fff; font-size:10px; padding:1px 6px; border-radius:20px;'>-" . $product['descuento'] . "%</span>"
